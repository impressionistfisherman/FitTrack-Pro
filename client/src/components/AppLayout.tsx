...
function getVisibleNavItems({
  ...
  if (isTrainer && location.startsWith("/trainer")) {
    if (location.startsWith("/trainer/clients/")) {
      ...
      return [
        ...
        {
          id: "trainer-client-workouts",
          href: trainerClientPath,
          icon: Dumbbell,
          label: "운동 기록",
        },
        {
  ...
  return userNavItems.map(item =>
    item.href === "/coaching" ? { ...item, badge: coachingBadge } : item
  );
}
...
function AppContentSkeleton() {
  return (
    <div
      className="page-shell page-shell-wide space-y-4"
      aria-label="앱 정보를 불러오는 중"
    >
  ...
  return (
    <a href={href} onClick={handleClick} className="block">
      ...
      >
        ...
        {isActive && (
          <span className={cn("text-primary text-xs", !badge && "ml-auto")}>
            ›
          </span>
        )}
      </div>
        ...
        >
          ...
          {showTrainer ? (
            <a
              href="/trainer"
              onClick={event =>
                handleInternalNavigation({ event, href: "/trainer", navigate })
              }
            >
              <Button
                size="sm"
          ...
          {showAdmin ? (
            <a
              href="/admin"
              onClick={event =>
                handleInternalNavigation({ event, href: "/admin", navigate })
              }
            >
              <Button
                size="sm"
...
function UserAvatar({
  ...
  return (
    <Avatar
      className={cn(
        "shrink-0 border border-primary/30 bg-primary/10",
        className
      )}
    >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold text-foreground">
                      {isAuthenticated ? displayName : "로그인이 필요합니다"}
                    </div>
