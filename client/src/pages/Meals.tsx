...
export default function Meals() {
  ...
  const targetsQuery = trpc.meals.targets.useQuery(undefined, {
    enabled: isAuthenticated,
    staleTime: 5 * 60_000,
  });
  const hasSavedMealTargets = Boolean(targetsQuery.data?.saved);
  const recommendedTargetsQuery = trpc.meals.recommendedTargets.useQuery(
    undefined,
    {
      enabled:
        isAuthenticated &&
        activeMealTab === "targets" &&
  ...
  const saveTargets = trpc.meals.saveTargets.useMutation({
    onSuccess: async savedTargets => {
      ...
      const invalidations = [utils.meals.recommendedTargets.invalidate()];
      if (!isWeeklyReportCollapsed) {
        invalidations.push(
          utils.meals.weeklyReport.invalidate({ endDate: date, days: 7 })
        );
  ...
  const parseMealImage = trpc.meals.parseMealImage.useMutation({
    onSuccess: result => {
      ...
      if (result.items.length)
        toast.success("이미지에서 음식을 찾았습니다. 확인 후 저장하세요.");
      else
        toast.warning("음식을 찾지 못했습니다. 더 선명한 사진을 사용하세요.");
    },
  ...
  const getServingSizeGrams = (food: any) =>
    Math.max(1, Math.round(Number(food?.servingSizeGrams) || 100));

  const calculateMealAmountGrams = (
    quantity: string,
    unit: string,
    food: any
  ) => {
  ...
  const updatePortionAmount = (quantity: string) => {
    ...
    setAmount(
      String(calculateMealAmountGrams(quantity, portionUnit, selectedFood))
    );
  };

  const updatePortionUnit = (unit: string) => {
    const currentGrams = Math.max(
      0,
      Number(amount) ||
        calculateMealAmountGrams(portionAmount, portionUnit, selectedFood)
    );
    const nextQuantity =
      unit === "g"
        ...
        : String(
            Math.round(
              (currentGrams / getServingSizeGrams(selectedFood)) * 10
            ) / 10 || 1
          );
    setPortionUnit(unit);
    setPortionAmount(nextQuantity);
    setAmount(
      String(calculateMealAmountGrams(nextQuantity, unit, selectedFood))
    );
  };
  ...
  if (!isAuthenticated) {
    return (
      <AuthRequiredState
        icon={Utensils}
        description="식단을 기록하고 영양 합계를 확인하려면 로그인하세요."
      />
    );
  }
    ...
    <div className="mobile-search-results mobile-food-search-results space-y-2 overflow-y-auto overscroll-contain">
      ...
      {foodsQuery.isError ? (
        <div className="rounded-xl border border-dashed border-destructive/40 bg-destructive/5 p-4 text-center text-sm text-destructive">
          음식 검색 API 응답을 받지 못했습니다. 잠시 후 다시 입력하거나 직접
          음식 등록을 사용하세요.
        </div>
      ) : null}
        ...
        <div
          ...
          className={cn(
            ...
            selectedFood?.id === food.id
              ? "food-search-option-selected"
              : "food-search-option-default"
          )}
        >
          <button
            type="button"
            ...
            <div className="flex shrink-0 items-center gap-1">
              ...
              <button
                ...
                onClick={event => {
                  ...
                  if (
                    !window.confirm(
                      "직접 등록한 음식을 삭제할까요? 기존 식단 기록은 유지됩니다."
                    )
                  )
                    return;
      ...
      !foodsQuery.data?.length ? (
        <div className="rounded-xl border border-dashed border-border p-4 text-center text-sm text-muted-foreground">
          검색 결과가 없습니다. 표기가 다르면 예: 육개장, 컵라면처럼 다시
          검색하거나 음식 등록으로 추가하세요.
        </div>
      ) : null}
    </div>
      ...
      <div className="page-content-header">
        <div>
          ...
          <p className="page-description">
            먹은 음식과 중량을 기록하고 하루 영양 합계를 확인하세요
          </p>
        </div>
              ...
              <Card className="border-primary/20 bg-primary/5">
                <CardContent className="p-5">
                  ...
                  <p className="mb-3 text-xs text-muted-foreground">
                    사진 분석 결과는 추정값입니다. 음식명과 중량을 확인한 뒤
                    저장하세요.
                  </p>
                  <input
                            ...
                            <Input
                              ...
                              onChange={event =>
                                setImageItems(items =>
                                  items.map((next, itemIndex) =>
                                    itemIndex === index
                                      ? { ...next, amount: event.target.value }
                                      : next
                                  )
                                )
                              }
                  ...
                  <CardContent className="space-y-3 p-5">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={e =>
                            setFoodForm(f => ({ ...f, brand: e.target.value }))
                          }
                          className="border-border bg-card"
                          placeholder="선택"
                        />
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setFoodForm(f => ({ ...f, aliases: e.target.value }))
                        }
                        className="border-border bg-card"
                        placeholder="닭찌, 훈제닭 처럼 쉼표로 구분"
                      />
                    </div>
                    <Button
                      className="w-full bg-primary text-primary-foreground"
                      disabled={!foodForm.name.trim() || createFood.isPending}
                      onClick={saveFood}
                    >
                      음식 저장
                    </Button>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-center">
                        <div className="rounded-xl bg-accent/45 p-3">
                          <div className="text-lg font-black text-foreground">
                            {weeklyReportQuery.data?.average.calories ?? 0}
                          </div>
                          <div className="text-[11px] text-muted-foreground">
                            평균 kcal
                          </div>
                        ...
                        <div className="rounded-xl bg-accent/45 p-3">
                          <div className="text-lg font-black text-foreground">
                            {weeklyReportQuery.data?.average.protein ?? 0}g
                          </div>
                          <div className="text-[11px] text-muted-foreground">
                            평균 단백질
                          </div>
                          ...
                          >
                            <div className="mb-2 flex items-center justify-between text-xs">
                              ...
                              <span className="text-muted-foreground">
                                {day.calories} /{" "}
                                {weeklyReportQuery.data.targets.calories} kcal
                              </span>
                            </div>
                            <div className="h-2 overflow-hidden rounded-full bg-accent">
                              <div
                                className="h-full rounded-full bg-primary"
                                style={{
                                  width: `${Math.min(100, targetPercent(day.calories, weeklyReportQuery.data.targets.calories))}%`,
                                }}
                              />
                            </div>
                          </div>
                ...
                <CardContent className="p-5">
                  ...
                  <div className="mb-4 rounded-xl border border-border bg-background/35 p-3 text-xs text-muted-foreground">
                    ...
                    {recommendedTargetsQuery.data?.basis ? (
                      <div className="mt-2 space-y-1 text-foreground">
                        <div>
                          운동 목표:{" "}
                          {recommendedTargetsQuery.data.basis.goalSummary} · 주{" "}
                          {recommendedTargetsQuery.data.basis.weeklyWorkouts}회
                          · 추천 전략:{" "}
                          {recommendedTargetsQuery.data.basis.label}
                        </div>
                        <div className="text-muted-foreground">
                          계산 기준: 체중{" "}
                          {recommendedTargetsQuery.data.basis.latestWeight}kg ·
                          BMR {recommendedTargetsQuery.data.basis.bmr}kcal ·
                          TDEE {recommendedTargetsQuery.data.basis.tdee}kcal
                        </div>
                      </div>
                    ) : null}
                  </div>
                  {recommendedTargetsQuery.data?.targets ? (
                    <div className="mb-4 rounded-xl border border-primary/20 bg-primary/5 p-3">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {recommendedTargetsQuery.data.basis.description}
                          </div>
                        </div>
                        <Button
                          size="sm"
                      ...
                      <div className="grid grid-cols-4 gap-2 text-center text-xs">
                        <div className="rounded-lg bg-background/45 p-2">
                          <b>{recommendedTargetsQuery.data.targets.calories}</b>
                          <br />
                          kcal
                        </div>
                        <div className="rounded-lg bg-background/45 p-2">
                          <b>{recommendedTargetsQuery.data.targets.protein}g</b>
                          <br />
                          단백
                        </div>
                        <div className="rounded-lg bg-background/45 p-2">
                          <b>{recommendedTargetsQuery.data.targets.carbs}g</b>
                          <br />
                          탄수
                        </div>
                        <div className="rounded-lg bg-background/45 p-2">
                          <b>{recommendedTargetsQuery.data.targets.fat}g</b>
                          <br />
                          지방
                        </div>
                      </div>
                    ...
                    <div className="mb-4 rounded-xl border border-border bg-background/35 p-3">
                      ...
                      <div className="grid grid-cols-4 gap-2 text-center text-xs">
                        {["kcal", "단백", "탄수", "지방"].map(label => (
                          <div
                            key={label}
                            className="rounded-lg bg-background/45 p-2 text-muted-foreground"
                          >
                            --
                            <br />
                            {label}
                    ...
                    <div className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-border bg-background/35 p-3">
                      ...
                      >
                        {recommendedTargetsQuery.isFetching
                          ? "계산 중"
                          : "계산값 불러오기"}
                      </Button>
                    </div>
                  ...
                  <Button
                    ...
                    disabled={saveTargets.isPending || targetsQuery.isLoading}
                  >
                    <Save size={14} />
              ...
              <CardContent className="p-5">
                ...
                <div className="mt-4 rounded-xl border border-border bg-background/35 p-3">
                  ...
                  <div className="space-y-2">
                    {recentMealsQuery.data?.length ? (
                      recentMealsQuery.data.map((meal: any) => (
                        <button
                          key={meal.id}
                          type="button"
                ...
                <div className="mt-4 grid gap-3 lg:grid-cols-2">
                  <div className="rounded-xl border border-border bg-background/35 p-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {recentFoodsQuery.data?.length ? (
                        recentFoodsQuery.data.map((food: any) => (
                          <button
                            key={food.id}
                            type="button"
                            onClick={() => selectFood(food)}
                  ...
                  <div className="rounded-xl border border-border bg-background/35 p-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {frequentFoodsQuery.data?.length ? (
                        frequentFoodsQuery.data.map((food: any) => (
                          <button
                            key={food.id}
                            type="button"
                            onClick={() => selectFood(food)}
                  ...
                  <div className="mt-4 rounded-xl border border-border bg-background/45 p-3">
                    <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {portionUnit === "g"
                            ? "그램 기준으로 기록"
                            : `1인분 ${getServingSizeGrams(selectedFood)}g 기준`}
                        </div>
                      </div>
                      <div className="meal-amount-controls">
                        ...
                        <div className="space-y-1">
                          ...
                          <Select
                            value={portionUnit}
                            onValueChange={updatePortionUnit}
                          >
                    ...
                    <div className="mb-3 flex flex-wrap gap-2">
                      {Array.from(
                        new Set(
                          [
                            Number(selectedFood.servingSizeGrams) || 100,
                            50,
                            100,
                            150,
                            200,
                    ...
                    {selectedPreview && (
                      <div className="grid grid-cols-4 gap-2 text-center text-xs">
                        <div className="rounded-lg bg-accent/45 p-2">
                          <b>{selectedPreview.calories}</b>
                          <br />
                          kcal
                        </div>
                        <div className="rounded-lg bg-accent/45 p-2">
                          <b>{selectedPreview.protein}g</b>
                          <br />
                          단백질
                        </div>
                        <div className="rounded-lg bg-accent/45 p-2">
                          <b>{selectedPreview.carbs}g</b>
                          <br />
                          탄수
                        </div>
                        <div className="rounded-lg bg-accent/45 p-2">
                          <b>{selectedPreview.fat}g</b>
                          <br />
                          지방
                        </div>
                      </div>
                ...
                <Textarea
                  ...
                  className="mt-3 min-h-20 border-border bg-accent"
                  placeholder="메모 선택"
                />
                <Button
                  className="mt-3 w-full bg-primary text-primary-foreground"
                  disabled={!selectedFood || createLog.isPending}
                  onClick={saveMeal}
                >
                  식단 저장
                </Button>
            ...
            <Card className="border-border bg-card">
              <CardContent className="p-5">
                ...
                <div className="space-y-3">
                  {mealsQuery.data?.meals?.length ? (
                    mealsQuery.data.meals.map((meal: any) => (
                      <div
                        key={meal.id}
                        className="rounded-xl border border-border bg-background/45 p-3"
                      >
